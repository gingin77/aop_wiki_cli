...
from aop_wiki_cli.utilities import set_up_logger, write_dict_to_json, get_dated_cache_dir
from aop_wiki_cli.analysis import (
...
from aop_wiki_cli.data_export import (
    write_csv,
...
from aop_wiki_cli.parsers import (
...
from aop_wiki_cli.configs.harmonize_ker_evidence import (
    AOPS_SELECTED_FOR_HARMONIZED_KERS_WORKBOOKS, CONCORDANCE_SEARCH_PARAMS
)
from aop_wiki_cli.harmonization import harmonize_kers_with_cache
from aop_wiki_cli.paths import (
    cache_root,
    curated_input_path,
    ensure_dir,
    outputs_dir,
    seizure_workbook_path,
    set_data_dir,
    ENV_VAR as DATA_DIR_ENV_VAR,
)

app = typer.Typer(pretty_exceptions_enable=False)

# Shared logger for all CLI commands. Handlers are attached in the callback,
# once the data directory (and so the log directory) is known.
logger = logging.getLogger('aop-wiki-cli')
...
def main(
    data_dir: Optional[Path] = typer.Option(
        ...
        "--data-dir",
        help=(
            "Directory to read inputs from and write outputs, caches and logs to. "
            f"Defaults to ${DATA_DIR_ENV_VAR} if set, otherwise the current directory."
        )
    )
):
    """AOP-Wiki CLI - extract and analyze Adverse Outcome Pathway data."""
    if data_dir is not None:
        set_data_dir(data_dir)

    # Configure logging now that the log directory is resolvable
    global logger
...
def collect_event_integration_rankings(
    ...
    output_dir = ensure_dir(outputs_dir('event_rankings'))
...
def collect_ker_analytics(
    ...
    cached under <data-dir>/outputs/cache/<date>/ for later KER commands to reuse.
    """
    # Setup
    work_date = datetime.strptime(cache_date, '%m-%d-%Y').date() if cache_date else today
    work_date_str = work_date.strftime('%m-%d-%Y')
...
def find_kers_for_events(
    ...
    work_date_str = work_date.strftime('%m-%d-%Y')
    cache_dir = get_dated_cache_dir(cache_root(), work_date)
    output_dir = outputs_dir('ker_lookups', work_date_str)
...
def search_kers_for_concordance_text(
    ...
    work_date_str = work_date.strftime('%m-%d-%Y')
    cache_dir = get_dated_cache_dir(cache_root(), work_date)
    output_dir = ensure_dir(outputs_dir('ker_evidence'))
...
def search_with_config(
    config: str = typer.Argument(
        ...,
        help="Config module name from aop_wiki_cli.configs (e.g., 'lung_and_immune_aops', 'regulatory_relevance', 'methods_nams')"
    ),
    cache_date: Optional[str] = typer.Option(
        None,
        "--date",
    ...
    Example:
        aop-wiki-cli search-with-config lung_and_immune_aops
        aop-wiki-cli search-with-config lung_and_immune_aops --co-occurrence-only
    """
...
def collect_harmonized_seizure_aops(
    ...
    1. Parses the seizure AOP workbook (behl_seizure_supp_data.xlsx, shipped with the tool
       unless overridden at <data-dir>/inputs/seizure_aops/)
    ...
    Caching:
        The workflow uses the curated review files shipped with the tool unless copies
        exist in <data-dir>/curated/, which take precedence. If a curated file is found,
        interactive review is skipped. Use --skip-curated to bypass curated inputs and run
        interactive review.
    
    Outputs:
        Results are written to <data-dir>/outputs/seizure_aops/{date}/ including:
    ...
    Examples:
        ...
        aop-wiki-cli collect-harmonized-seizure-aops --date 03-12-2026
        aop-wiki-cli collect-harmonized-seizure-aops --skip-curated --force-refresh
    ...
    work_date_str = work_date.strftime('%m-%d-%Y')
    cache_dir = get_dated_cache_dir(cache_root(), work_date)
    output_dir = ensure_dir(outputs_dir('seizure_aops'))
    ...
    path_to_reviewed_matches = curated_input_path(
        "reviewed_ke_description_to_harmonized_ke_mapping.json"
    )
    if not skip_curated and path_to_reviewed_matches.exists():
    ...
    path_to_curated_events_to_target_families = curated_input_path(
        "curated_event-target_family_mappings.json"
    )
    if not skip_curated and path_to_curated_events_to_target_families.exists():
...
def manually_review_matches(
    ...
    aop-wiki-cli manually-review-matches <data-dir>/outputs/seizure_aops/{date}/mapping_ke_description_to_harmonized_ke_{date}.json --threshold 0.9
...
def _available_search_configs():
    """Names of packaged config modules usable with `search-with-config`.

    A config qualifies by defining both SEARCH_PARAMS and OUTPUT_CONFIG, which
    is what this command reads; the other modules under configs/ hold settings
    for other workflows.
    """
    import pkgutil
    from aop_wiki_cli import configs as configs_package

    usable = []
    for module_info in pkgutil.iter_modules(configs_package.__path__):
        try:
            module = importlib.import_module(f'aop_wiki_cli.configs.{module_info.name}')
        except ImportError:
            continue
        if hasattr(module, 'SEARCH_PARAMS') and hasattr(module, 'OUTPUT_CONFIG'):
            usable.append(module_info.name)
